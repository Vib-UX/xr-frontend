...
const account = privateKeyToAccount(process.env.NEXT_PUBLIC_SFUEL_PK as `0x${string}`);
...
function useSendGasFunds() {
  ...
  return useQuery({
    queryKey: ['send-gas-funds', chain?.id, !!publicClient, morphBiconomyAccountAddress],
    ...
    queryFn: async () => {
      try {
        ...
        const nativeAmount = AMOUNT_OF_USDC_TO_SEND[chain.id as keyof typeof AMOUNT_OF_USDC_TO_SEND].native.toString();
        const usdcAmount = AMOUNT_OF_USDC_TO_SEND[chain.id as keyof typeof AMOUNT_OF_USDC_TO_SEND].usdc.toString();
        if (Big(balance.toString()).eq(0) && Big(usdcBalance.toString()).eq(0)) {
          ...
          return {
            nativeAmount: Big(nativeAmount).div(10 ** 18).toString(),
            usdcAmount: Big(usdcAmount).div(10 ** 6).toString()
          }
        }
