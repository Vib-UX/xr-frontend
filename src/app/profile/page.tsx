...
export default function ProfilePage() {
  ...
  const handleGetFitRedirection = async () => {
    ...
    window.location.href = `https://www.fitbit.com/oauth2/authorize?response_type=code&client_id=23PW5Y&scope=activity+cardio_fitness+electrocardiogram+heartrate+irregular_rhythm_notifications+location+nutrition+oxygen_saturation+profile+respiratory_rate+settings+sleep+social+temperature+weight&code_challenge=${challenge}&code_challenge_method=S256`;
  };
  return (
          ...
          <div>
            {[0, 1].map(() => (
              <div className="pt-3 flex items-center gap-x-4">
                <Image src={Gunna} alt="Coach" className="object-cover" />
          ...
          {WorkoutData.map((elem, index) => (
            ...
            >
              ...
              <div className="flex items-center justify-between mt-5">
                ...
                <Image src={elem.img} alt="tracker" height={40} width={40} />
              </div>
            </div>
          ))}
        </div>
