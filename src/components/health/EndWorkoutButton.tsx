...
const EndWorkoutButton = ({
    ...
    const handleRecordDailyWorkout = async () => {
        try {
            ...
            const recordDailyWorkoutTxHash = await writeContractAsync({
                address: FITNESS_ADDRESS_MAPPING[
                    chain?.id as keyof typeof FITNESS_ADDRESS_MAPPING
                ] as `0x${string}`,
                abi: FITNESS_ABI,
                functionName: 'recordDailyWorkout',
            });
            const recordDailyWorkoutTxReceipt =
    ...
    return (
        <div className=" w-full">
            <Button
                onClick={() => withToast(handleRecordDailyWorkout)}
                disabled={disabled}
            ...
            {txHash && (
                <div className="flex items-center justify-end">
                    <a
                        ...
                        target="_blank"
                        className="text-sm text-gray-500 hover:underline text-end mt-2"
                    >
                        View transaction
                    </a>
                </div>
            ...
            {attestaionId && (
                <div className="flex items-center justify-start">
                    <a
                        href={`https://testnet-scan.sign.global/attestation/onchain_evm_80002_${attestaionId}`}
                        target="_blank"
                        className="text-sm text-gray-500 hover:underline text-end mt-2"
                    >
                        View Attestation
